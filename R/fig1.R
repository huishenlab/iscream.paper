...
fig1 <- function(
  ...
  # Figure 1 B
  bsseq_sc <- fread(bsseq_results$sc$data)[,
    thread_count := as.factor(thread_count)
  ]
  query_all_sc <- fread(query_all_results$sc$data)[,
    thread_count := as.factor(thread_count)
  ]
  biscuiteer_sc <- fread(biscuiteer_results$sc$data)[,
    thread_count := as.factor(thread_count)
  ]

  all_bsseq_sc <- rbind(bsseq_sc, query_all_sc, biscuiteer_sc)[
    thread_count %in% c(1, 16)
  ]
  ...
  bsseq_bulk <- fread(bsseq_results$bulk$data)[,
    thread_count := as.factor(thread_count)
  ]
  query_all_bulk <- fread(query_all_results$bulk$data)[,
    thread_count := as.factor(thread_count)
  ]
  biscuiteer_bulk <- fread(biscuiteer_results$bulk$data)[,
    thread_count := as.factor(thread_count)
  ]
