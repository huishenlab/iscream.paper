get_runtime_diff <- function(sc = TRUE, file_count = 90) {
  ...
  runtime_dt[, time.mean := mean(time), by = .(package, file_count, thread_count)]

  iscream_means <- runtime_dt[package == 'iscream', .(iscream.min = min(time.mean)), by = .(package, thread_count, file_count)]
  (
...
plot_runtime_diff <- function(sc = TRUE, file_count = 90) {
  ...
  thread_colors_fill <<- scale_fill_manual(values = c("16" = "#E66100", "1" = "#5D3A9B"))
