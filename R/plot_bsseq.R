...
#'
#' @importFrom ggplot2 ggplot aes geom_point stat_summary labs expand_limits scale_color_manual theme_bw guides guide_legend
#' @export
plot_bsseq <- function(
  benchmark,
  ...
  plot_data <- benchmark
  if (
    (files_or_regions == "files" & "region_count" %in% colnames(benchmark)) ||
      (files_or_regions == "regions" & "region_count" %in% colnames(benchmark))
  ) {
    plot_data <- benchmark[get(cap_axis) == other_max_count]
  }

  plot_data[, thread_count := as.factor(thread_count)]
  ...
  p <- ggplot(
    ...
    stat_summary(
      aes(
        y = time,
        group = interaction(package, thread_count),
        linetype = thread_count
      ),
      fun = mean,
      geom = "line",
    ...
    geom_point(alpha = alpha, show.legend = TRUE, size = dot_size) +
    expand_limits(y = 0) +
    labs(
      y = "Runtime (seconds)",
